...
/**
 * @api {get} /v4/findMy/:resource+/?query=params List
 * @apiVersion 4.0
 * @apiGroup Entities
 * @apiName List
 * @apiSampleRequest /v4/findMy/resource/name/?query=params
 */
router.get(`/${api.v4}/findMy/:resource+/`, async (ctx) => {
  const resource = ctx.params.resource;
  const user = ctx.req.user || { id: 1, email: roles.anonymous, role: roles.anonymous };
  checkPerms(ctx.params.resource, user.role, "readOwn");
